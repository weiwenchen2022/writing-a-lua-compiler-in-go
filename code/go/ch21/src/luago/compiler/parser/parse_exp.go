...
func parseExp11(lexer *Lexer) Exp {
	...
	if lexer.LookAhead()== TOKEN_OP_AND {
		line, op, _ := lexer.NextToken()
...
func parseExp10(lexer *Lexer) Exp {
	...
	for {
		...
		case TOKEN_OP_LT, TOKEN_OP_GT, TOKEN_OP_LE,
			TOKEN_OP_GE, TOKEN_OP_NE, TOKEN_OP_EQ:
			line, op, _ := lexer.NextToken()
