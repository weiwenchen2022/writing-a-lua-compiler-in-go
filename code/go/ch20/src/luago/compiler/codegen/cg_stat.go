...
func cgStat(fi *funcInfo, node Stat) {
	...
	case *LabelStat, *GotoStat:
		panic("label and goto statements not supported!")
	}
}
...
func cgLocalVarDeclStat(fi *funcInfo, node *LocalVarDeclStat) {
	...
	} else if nNames < nExps {
		for i, exp := range exps {
			a := fi.allocReg()
			if nExps - 1 == i && isVarargOrFuncCall(exp) {
				cgExp(fi, exp, a, 0)
			} else {
				cgExp(fi, exp, a, 1)
			}
...
func cgAssignStat(fi *funcInfo, node *AssignStat) {
	...
	if nVars <= nExps {
		for i, exp := range exps {
			a := fi.allocReg()
			if i >= nVars && nExps - 1 == i && isVarargOrFuncCall(exp) {
				cgExp(fi, exp, a, 0)
			} else {
				cgExp(fi, exp, a, 1)
			}
