...
func coResume(ls LuaState) int {
	...
	if r := _auxResume(ls, co, ls.GetTop() - 1); r < 0 {
		...
		ls.PushBoolean(true)
		ls.Insert(-(r + 1))
		return r + 1 /* return true plus 'resume' returns */
	}
}
...
func _auxResume(ls, co LuaState, narg int) int {
	...
	if LUA_OK == co.Status() && co.GetTop() == 0 {
...
// lua-5.3.5/src/lcorolib.c#luaB_yield()
...
// lua-5.3.5/src/lcorolib.c#luaB_costatus()
	...
	if ls == co {
		...
		case LUA_OK:
			if co.GetStack() { /* does it have frames? */
				ls.PushString("normal") /* it is active */
			} else if co.GetTop() == 0 {
				ls.PushString("dead")
			} else {
				ls.PushString("suspended")
...
func _auxWrap(ls LuaState) int {
	co := ls.ToThread(LuaUpvalueIndex(1))
	ls.ArgCheck(co != nil, 1, "thread expected")

	if r := _auxResume(ls, co, ls.GetTop()); r < 0 {
		return ls.Error()
	} else {
		return r
	}
