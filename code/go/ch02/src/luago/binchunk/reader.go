...
func (self *reader) checkHeader() {
	if LUA_SIGNATURE != string(self.readBytes(4)) {
		panic("not a precompiled chunk!")
	}

	if LUAC_VERSION != self.readByte() {
		panic("version mismatch!")
	}

	if LUAC_FORMAT != self.readByte() {
		panic("format mismatch!")
	}

	if LUAC_DATA != string(self.readBytes(6)) {
		panic("corrupted!")
	}

	if CINT_SIZE != self.readByte() {
		panic("int size mismatch!")
	}

	if CSIZET_SIZE != self.readByte() {
		panic("size_t mismatch!")
	}

	if INSTRUCTION_SIZE != self.readByte() {
		panic("instruction size mismatch!")
	}

	if LUA_INTEGER_SIZE != self.readByte() {
		panic("lua_Integer size mismatch!")
	}

	if LUA_NUMBER_SIZE != self.readByte() {
		panic("lua_Number size mismatch!")
	}

	if LUAC_INT != self.readLuaInteger() {
		panic("endianness mismatch!")
	}

	if LUAC_NUM != self.readLuaNumber() {
...
func (self *reader) readContants() []interface{} {
	constants := make([]interface{}, self.readUint32())
	for i := range constants {
		constants[i] = self.readContant()
	}

	return constants
}

func (self *reader) readContant() interface{} {
