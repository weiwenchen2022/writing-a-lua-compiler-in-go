...
func parseExpList(lexer *Lexer) []Exp {
	...
	for TOKEN_SEP_COMMA == lexer.LookAhead() {
		lexer.NextToken() // ','
		exps = append(exps, parseExp(lexer)) // exp
	}
...
func parseExp12(lexer *Lexer) Exp {
	...
	for TOKEN_OP_OR == lexer.LookAhead() {
...
func parseExp11(lexer *Lexer) Exp {
	...
	for TOKEN_OP_AND == lexer.LookAhead() {
		line, op, _ := lexer.NextToken()
...
func parseExp9(lexer *Lexer) Exp {
	...
	for TOKEN_OP_BOR == lexer.LookAhead() {
		line, op, _ := lexer.NextToken()
...
func parseExp8(lexer *Lexer) Exp {
	...
	for TOKEN_OP_BXOR == lexer.LookAhead() {
		line, op, _ := lexer.NextToken()
...
func parseExp7(lexer *Lexer) Exp {
	...
	for TOKEN_OP_BAND == lexer.LookAhead() {
		line, op, _ := lexer.NextToken()
...
func parseExp5(lexer *Lexer) Exp {
	...
	line := 0
	exps := []Exp{exp,}
	for TOKEN_OP_CONCAT == lexer.LookAhead() {
...
func parseExp1(lexer *Lexer) Exp {
	...
	if TOKEN_OP_POW == lexer.LookAhead() {
		line, op, _ := lexer.NextToken()
...
// exp0 ::= nil | false | true | Numeral | LiteralString
// | '...'' | functiondef | prefiexexp | tableconstructor
...
func _parseParList(lexer *Lexer) (names []string, isVararg bool) {
	...
	for TOKEN_SEP_COMMA == lexer.LookAhead() {
		lexer.NextToken() // ','
		if TOKEN_IDENTIFIER == lexer.LookAhead() {
			_, name := lexer.NextIdentifier()
			names = append(names, name)
		} else {
...
func _isFieldSep(tokenKind int) bool {
	return TOKEN_SEP_COMMA == tokenKind || TOKEN_SEP_SEMI == tokenKind
}
